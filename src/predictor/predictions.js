...
class PDF {
  ...
  decay(rate_decay_min, rate_decay_max) {
    const ret = new PDF(this.min_value() - rate_decay_max, this.max_value() - rate_decay_min, false);
    /*
...
class Predictor {
  ...
  rate_range_from_given_and_base(given_price, buy_price) {
    return [this.minimum_rate_from_given_and_base(given_price, buy_price), this.maximum_rate_from_given_and_base(given_price, buy_price)];
  }
  ...
  *multiply_generator_probability(generator, probability) {
  ...
  generate_peak_price(given_prices, predicted_prices, start, rate_min, rate_max) {
    ...
    if (!isNaN(middle_price)) {
      ...
      // TODO: How to deal with probability when there's fudge factor?
      // Clamp the value to be in range now so the probability won't be totally biased to fudged values.
      const real_rate_range = this.rate_range_from_given_and_base(clamp(middle_price, min_pred, max_pred), buy_price);
      prob *= range_intersect_length(rate_range, real_rate_range) / range_length(rate_range);
      if (prob == 0) {
        return 0;
      }
